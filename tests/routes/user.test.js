...
describe('User Integration tests', () => {
	...
	describe('users.get cool stuff', ()  => {
		...
		it('Should return an array of all users', (done) => {
			...
			agent
			.get('/api/users')
			// we send a get request to /api/users
			.end((err,res) => {
	...
	describe('users.put', ()  => { 
		it('Should be able to create (PUT) a user', (done) => {
			// Given (preconditions)
			userMock
			.expects('updateOne')
			...
			// When (someting happens)
			agent
			.put('/api/users/5cecf112a66bc43a217dfda3')
			.send(request)
			.end((err,res) => {
			// Then (something should happen)
				expect(res.status).to.equal(201);
	...
	describe('users.delete', ()  => { 
		it('Should be able to delete a user', (done) => {
			...
			// When (someting happens)
			agent
			.delete('/api/users/5cecf112a66bc43a217dfda3')
			.send()
			.end((err,res) => {
			// Then (something should happen)
				expect(res.status).to.equal(200);
